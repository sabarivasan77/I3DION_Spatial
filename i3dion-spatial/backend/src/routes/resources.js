...
resourcesRouter.get('/me', (req, res) => res.json({ user: req.user }));
  ...
  asyncHandler(async (req, res) => {
    ...
    if (newPassword) {
      ...
      await query(
        ...
        [name ?? null, email ?? null, phone ?? null, avatarUrl ?? null, passwordHash, req.user.id, req.user.company_id],
      );
    } else {
      await query(
        `UPDATE users
        ...
        [name ?? null, email ?? null, phone ?? null, avatarUrl ?? null, req.user.id, req.user.company_id],
      );
    }

    const { rows } = await query('SELECT id, company_id, name, email, phone, avatar_url, role FROM users WHERE id = $1', [
      req.user.id,
    ]);
    res.json({
      user: {
