...
async function offlineFallback<T>(path: string, options: RequestInit & { token?: string; formData?: FormData }) {
  ...
  if (path === '/company') {
    return {
      id: 'offline-company',
      name: 'I3DION Industrial Solutions',
      website: 'https://i3dion.com',
      logo_url: '',
      primary_color: '#2563EB',
      profile: 'Offline preview workspace',
    } as T;
  }
  if (path === '/uploads') {
    return {
...
export const api = {
  ...
  updateMe: (
    token: string,
    payload: { name?: string; email?: string; phone?: string; avatarUrl?: string; currentPassword?: string; newPassword?: string },
  ...
  createLead: (payload: unknown) => apiRequest('/leads', { method: 'POST', body: JSON.stringify(payload) }),
