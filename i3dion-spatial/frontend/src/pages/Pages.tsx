    ...
    <main className="pt-16">
      <section className="relative overflow-hidden px-4 py-20 md:px-6 md:py-28">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-2">
          <div className="relative z-10">
            <p className="mb-6 inline-flex items-center gap-2 rounded-full bg-primary px-3 py-1 text-sm font-semibold text-white"><Sparkles size={16} />Enterprise AR V3.0 is Live</p>
            <h1 className="text-4xl font-bold leading-tight text-slate-950 md:text-6xl">Industrial AR Catalog &amp; <span className="text-primary">Sales Visualization</span> Platform</h1>
            <p className="mt-6 max-w-xl text-lg leading-8 text-slate-600">Empower sales teams with high-fidelity augmented reality experiences that convert complex industrial specs into interactive product stories.</p>
          ...
          <div className="relative">
            ...
            <Card className="relative overflow-hidden rounded-3xl">
              <div className="flex items-center justify-between border-b border-slate-100 p-4">
                ...
                <p className="text-sm font-semibold text-primary">Live AR Preview: Pump X-1</p>
              </div>
      ...
      <section id="features" className="bg-white px-4 py-20 md:px-6">
        <div className="mx-auto max-w-7xl">
          <SectionTitle title="Features" meta="Built for industrial sales cycles" />
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {features.map((f) => (<Card key={f} className="p-6"><Layers className="mb-5 text-primary" size={28} /><h3 className="text-lg font-semibold">{f}</h3><p className="mt-3 text-sm leading-6 text-slate-500">Designed for product teams that need precise visualization and measurable engagement.</p></Card>))}
          </div>
        </div>
      </section>
      <section id="industries" className="px-4 py-20 md:px-6">
        <div className="mx-auto max-w-7xl">
          <SectionTitle title="Industries" />
          <div className="grid gap-4 md:grid-cols-4">
            {industries.map((i) => (<div key={i} className="rounded-2xl border border-slate-200 bg-slate-50 p-6 text-lg font-semibold">{i}</div>))}
          </div>
        </div>
      </section>
      <section className="px-4 py-20 md:px-6">
        <div className="mx-auto max-w-7xl rounded-3xl bg-navy p-8 text-white md:p-12">
          <h2 className="text-3xl font-bold md:text-4xl">Ready to spatialize your product catalog?</h2>
          <p className="mt-3 max-w-2xl text-slate-300">Launch a measurable AR sales experience for complex industrial products.</p>
          <Link to="/signup" className="mt-8 inline-flex h-12 items-center justify-center rounded-xl bg-primary px-5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-blue-600 active:scale-[0.98]">
            Start Workspace
          </Link>
        </div>
      </section>
      <footer className="border-t border-slate-200 bg-white px-4 py-8 text-sm text-slate-500 md:px-6">
        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-4 md:flex-row">
          ...
          <p>Industrial AR Catalog &amp; Sales Visualization Platform</p>
        </div>
      </footer>
    </main>
  );
...
export function DashboardPage() {
  ...
  const kpis = useMemo(() => {
    const totalViews = SAMPLE_ANALYTICS.reduce((a, b) => a + b.views, 0);
    const totalLeads = summary ? summary.leads.reduce((a: number, l: any) => a + l.count, 0) : 0;
    return [
      { label: 'Total Products', value: String(products.length || 0), change: '+0%', icon: Box, tone: 'neutral' as const },
      { label: 'Total Views', value: totalViews.toLocaleString(), change: '+12%', icon: Eye, tone: 'positive' as const },
  ...
  async function saveLead(data: any) {
    ...
    try {
      if (editLead) {
        ...
        const saved: any = await api.createLead(data);
        const mapped = mapLeadRow(saved);
        setLeads((c) => [mapped, ...c]);
