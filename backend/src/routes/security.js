...
securityRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [usersRes, failedLoginsRes, alertsRes] = await Promise.all([
      query('SELECT count(*) as total FROM users WHERE organization_id = $1', [organization_id]),
      query('SELECT sum(failed_login_attempts) as total_failed FROM users WHERE organization_id = $1', [organization_id]),
      query('SELECT count(*) as active_alerts FROM security_alerts WHERE organization_id = $1 AND is_resolved = false', [organization_id]),
    ]);
  ...
  asyncHandler(async (req, res) => {
    const { rows } = await query(
      `SELECT s.id, s.device_info, s.ip_address, s.created_at, s.expires_at, u.email, u.name 
       ...
       WHERE u.organization_id = $1 AND s.is_revoked = false AND s.expires_at > now()
  ...
  asyncHandler(async (req, res) => {
    ...
    const { rows } = await query(
      `UPDATE user_sessions s
       SET is_revoked = true
       FROM users u
       WHERE s.id = $1 AND s.user_id = u.id AND u.organization_id = $2
       RETURNING s.id`,
