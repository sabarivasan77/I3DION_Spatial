...
export const TimelinePanel: React.FC = () => {
  ...
  const { experience, selectedWidgetId } = useStudioStore();
  ...
  if (isCollapsed) {
    return (
      <div className="bg-slate-900 border-t border-slate-800 h-9 flex items-center justify-between px-4 text-xs text-slate-400 select-none">
        <div className="flex items-center gap-2 font-mono text-cyan-400">
          <Film size={14} />
          <span className="font-semibold">{activeTimeline?.name || 'Timeline'}</span>
          <span className="text-slate-500">({currentTime.toFixed(2)}s / {duration.toFixed(2)}s)</span>
        </div>
        <button
          onClick={() => setIsCollapsed(false)}
          className="p-1 hover:bg-slate-800 rounded text-slate-300 transition flex items-center gap-1"
        >
          <ChevronUp size={14} />
  ...
  return (
    <div className="bg-slate-900 border-t border-slate-800 flex flex-col h-64 select-none text-slate-200">
      {/* Top Header & Controls */}
      <div className="h-10 px-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/60 shrink-0">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 font-mono text-cyan-400 text-xs font-bold mr-2">
            <Film size={15} />
          ...
          <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 rounded-lg p-0.5">
            {isPlaying ? (
              <button
                onClick={pause}
                title="Pause"
                className="p-1.5 hover:bg-slate-800 rounded text-amber-400 transition"
              >
                <Pause size={14} />
              </button>
            ) : (
              <button
                onClick={play}
                title="Play"
                className="p-1.5 hover:bg-slate-800 rounded text-emerald-400 transition"
              >
                <Play size={14} />
              </button>
            )}
            <button
              ...
              className="p-1.5 hover:bg-slate-800 rounded text-rose-400 transition"
            >
              <Square size={13} />
            </button>
            <button
          ...
          <div className="flex items-center gap-2 font-mono text-xs text-slate-300">
            ...
            <select
              ...
              className="bg-slate-900 border border-slate-800 rounded text-[11px] px-1.5 py-0.5 text-slate-300 ml-2"
            >
        ...
        <div className="flex items-center gap-2">
          {selectedWidgetId && (
            <div className="flex items-center gap-1">
              <span className="text-[11px] text-slate-400 mr-1 font-mono">Add Track:</span>
              <button
                onClick={() => handleAddTrackForSelectedWidget('position3D')}
                className="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 text-cyan-400 rounded text-[11px] font-mono border border-slate-700 transition"
              >
                + Position
              </button>
              <button
          ...
          <button
            ...
            className="p-1.5 hover:bg-slate-800 rounded text-slate-400 ml-2"
          >
            <ChevronDown size={15} />
          </button>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-y-auto">
          ...
          <div className="h-7 border-b border-slate-800 flex items-center bg-slate-950/80 px-2 text-[10px] font-mono text-slate-400">
            <div className="w-56 px-3 border-r border-slate-800 shrink-0">TARGET / PROPERTY</div>
          ...
          {tracks.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-slate-500 text-xs">
              <Film size={24} className="mb-2 opacity-40 text-cyan-500" />
              <p>No animation tracks in active timeline.</p>
              <p className="text-[11px] text-slate-600 mt-1">
                Select a 3D model or widget on canvas and click <span className="text-cyan-400 font-mono">+ Position / Rotation / Scale</span> above to start keyframing.
              </p>
            </div>
          ) : (
        ...
        {activeKfDetails && (
          <div className="w-64 border-l border-slate-800 bg-slate-950/90 p-3 text-xs flex flex-col justify-between shrink-0">
            <div>
              <div className="flex items-center justify-between border-b border-slate-800 pb-2 mb-2">
                <span className="font-bold text-amber-400 font-mono">KEYFRAME PROPERTIES</span>
                <button
              ...
              <div className="space-y-2 font-mono text-[11px]">
                <div>
                  <label className="text-slate-400 block mb-1">Time (seconds):</label>
                  <input
                    type="number"
                    step={0.1}
                ...
                <div>
                  <label className="text-slate-400 block mb-1">Interpolation:</label>
                  <select
